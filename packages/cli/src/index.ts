...
import type { ParsedArgs, MigrateCommandOptions, GenerateCommandOptions, DevCommandOptions } from './types';
    ...
    switch (args.command) {
      case 'migrate': {
        ...
        if (args.options['status']) {
          ...
          logger.info('TODO: Initialize adapter and migration runner from config');
          process.exit(0);
        }
      ...
      case 'generate': {
        ...
        const generateOptions: GenerateCommandOptions = {
          config: typeof genConfigValue === 'string' ? genConfigValue : undefined,
          verbose: Boolean(args.options['verbose']),
          output: typeof outputValue === 'string' ? outputValue : undefined,
        };

        const generateResult = await generateCommand(args.subcommand, name, generateOptions);
      ...
      case 'dev': {
        ...
        const devOptions: DevCommandOptions = {
          config: typeof devConfigValue === 'string' ? devConfigValue : undefined,
          verbose: Boolean(args.options['verbose']),
