 ...
 *   - UserBase extends ForjaEntry    (scalar fields only)
 *   - UserRelation                   (read-mode nested types)
 *   - UserRelationUpdate             (write-mode Relation<T> types)
 *   - type User = UserBase & UserRelation
 *   - type CreateUserInput = Omit<UserBase, keyof ForjaEntry> & UserRelationUpdate
...
import { scalarFieldToTypeString, isScalarField, isExcludedFromBase } from "./scalar-fields";
...
/**
 * Generate the UserBase interface (scalar fields + ForjaEntry)
 */
function generateBase(schema: SchemaDefinition, name: string): string {
	const lines: string[] = [];
	lines.push(`export interface ${name}Base extends ForjaEntry {`);
 ...
 */
function generateRelationWrite(
...
function generateTypeAliases(name: string): string {
	return [
		...
		`export type Create${name}Input = Omit<${name}Base, keyof ForjaEntry> & ${name}RelationUpdate;`,
		`export type Update${name}Input = Partial<Omit<${name}Base, keyof ForjaEntry>> & ${name}RelationUpdate;`,
	].join("\n");
}
 ...
 */
const INTERNAL_SCHEMA_NAMES = ["_forja_migration"] as const;
...
function isInternalSchema(schema: SchemaDefinition): boolean {
	...
	if ((INTERNAL_SCHEMA_NAMES as readonly string[]).includes(schema.name)) return true;
	return false;
}
