...
describe("QueryBuilder - Happy Path", () => {
	...
	describe("SELECT Query", () => {
		...
		it("should build select query with wildcard", () => {
			const wildcardQuery = selectFrom("users").select("*").build();
	...
	describe("DELETE Query", () => {
		it("should build delete query", () => {
			const deleteQuery = deleteFrom("users").where({ id: 1 }).build();
	...
	describe("ORDER BY", () => {
		...
		it("should support multiple order by", () => {
			...
			expect(queryData.orderBy?.[0]).toEqual({
				field: "role",
				direction: "asc",
			});
			expect(queryData.orderBy?.[1]).toEqual({
				field: "createdAt",
				direction: "desc",
			});
		});
	...
	describe("Pagination", () => {
		it("should set limit", () => {
			const limitedQuery = selectFrom("users").limit(10).build();
		...
		it("should set offset", () => {
			const offsetQuery = selectFrom("users").offset(20).build();
		...
		it("should support limit and offset together", () => {
			const paginatedQuery = selectFrom("users").limit(25).offset(50).build();
	...
	describe("Clone", () => {
		...
		it("should deep clone nested objects (where, populate, data)", () => {
			...
			const originalQueryData = expectSuccessData(
				originalNestedBuilder.build(),
			);
