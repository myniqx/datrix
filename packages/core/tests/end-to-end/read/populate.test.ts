...
describe("Populate", () => {
	...
	beforeAll(async () => {
		...
		const role1 = await datrix.create("role", { name: "Role A", level: 50 });
		const role2 = await datrix.create("role", { name: "Role B", level: 75 });
	...
	describe("Simple Populate", () => {
		...
		it("should populate with true shorthand", async () => {
			const dept = await datrix.findById("department", deptId, {
				populate: { organization: true },
			});
	...
	describe("ManyToMany Populate", () => {
		...
		it("should return empty array for manyToMany with no relations", async () => {
			...
			const result = await datrix.findById("user", user.id, {
				populate: { roles: true },
			});
	...
	describe("Self-Referencing Populate", () => {
		...
		beforeAll(async () => {
			...
			const grandchild = await datrix.create("category", {
		...
		it("should populate self-referencing relation", async () => {
			const child = await datrix.findById("category", childId, {
		...
		it("should populate nested self-reference", async () => {
			const grandchild = await datrix.findById("category", grandchildId, {
				populate: {
					parent: {
						populate: { parent: true },
					},
	...
	describe("HasOne Populate", () => {
		...
		beforeAll(async () => {
			...
			const favCategory = await datrix.create("category", {
			...
			const userWithFavCat = await datrix.create("user", {
		...
		it("should populate hasOne relation", async () => {
			const user = await datrix.findById("user", userWithFavCatId, {
		...
		it("should return null for hasOne when not set", async () => {
			...
			const userWithoutFav = await datrix.create("user", {
				email: "no-fav-cat@test.com",
				name: "No Favorite User",
			});

			const user = await datrix.findById("user", userWithoutFav.id, {
				populate: { favoriteCategory: true },
			});
		...
		it("should populate hasOne with nested relation", async () => {
			...
			await datrix.update("category", favCategoryId, {
				parent: parentCat.id,
			});
