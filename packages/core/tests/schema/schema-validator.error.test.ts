...
import { assertSchema, isValid, validateMany, validateOrThrow } from '../../src/validator/schema-validator';
import { invalidData, sampleSchemas, validData } from '../../../types/src/test/fixtures';
...
describe('SchemaValidator - Error Path', () => {
  describe('validateSchema - Type Checking', () => {
    it('should reject non-object data', () => {
      ...
      const validationResult = validateSchema(nonObjectInput, sampleSchemas.userSchema);
    ...
    it('should reject null', () => {
      ...
      const validationResult = validateSchema(nullInput, sampleSchemas.userSchema);
    ...
    it('should reject undefined', () => {
      ...
      const validationResult = validateSchema(undefinedInput, sampleSchemas.userSchema);
    ...
    it('should reject array', () => {
      ...
      const validationResult = validateSchema(arrayInput, sampleSchemas.userSchema);
    ...
    it('should reject number', () => {
      ...
      const validationResult = validateSchema(numberInput, sampleSchemas.userSchema);
  ...
  describe('validateSchema - Required Fields', () => {
    it('should fail when required fields are missing', () => {
      ...
      const validationError = expectFailureError(validationResult);
      expect(validationError.some(e => e.field === 'email' && e.code === 'REQUIRED')).toBe(true);
      expect(validationError.some(e => e.field === 'name' && e.code === 'REQUIRED')).toBe(true);
    });
  });
  ...
  describe('validateSchema - Field Validation', () => {
    it('should fail for invalid email format', () => {
      ...
      const validationError = expectFailureError(validationResult);
      expect(validationError.some(e => e.field === 'email' && e.code === 'PATTERN')).toBe(true);
    });
    ...
    it('should fail for age below minimum', () => {
      ...
      const validationError = expectFailureError(validationResult);
      expect(validationError.some(e => e.field === 'age' && e.code === 'MIN_VALUE')).toBe(true);
    });
    ...
    it('should fail for invalid enum value', () => {
      ...
      const validationError = expectFailureError(validationResult);
      expect(validationError.some(e => e.field === 'role' && e.code === 'INVALID_ENUM')).toBe(true);
    });
    ...
    it('should fail for title too short', () => {
      ...
      const validationError = expectFailureError(validationResult);
      expect(validationError.some(e => e.field === 'title' && e.code === 'MIN_LENGTH')).toBe(true);
    });
    ...
    it('should fail for title too long', () => {
      ...
      const validationError = expectFailureError(validationResult);
      expect(validationError.some(e => e.field === 'title' && e.code === 'MAX_LENGTH')).toBe(true);
    });
  });
  ...
  describe('validateSchema - Multiple Errors', () => {
    it('should collect all validation errors by default', () => {
      ...
      const validationResult = validateSchema(multipleErrorsData, sampleSchemas.userSchema);
  ...
  describe('validateSchema - Unknown Fields (Strict Mode)', () => {
    it('should fail for unknown fields in strict mode (default)', () => {
      ...
      const validationResult = validateSchema(userWithUnknownField, sampleSchemas.userSchema);

      const validationError = expectFailureError(validationResult);
      expect(validationError.some(e => e.field === 'extraField' && e.code === 'UNKNOWN')).toBe(true);
    });
  });
  ...
  describe('validatePartial - Update Validation', () => {
    it('should validate provided fields', () => {
      ...
      const validationResult = validatePartial(partialUpdateWithInvalidEmail, sampleSchemas.userSchema);
    ...
    it('should reject non-object data', () => {
      const nonObjectInput = 'not an object';
      const validationResult = validatePartial(nonObjectInput, sampleSchemas.userSchema);
    ...
    it('should handle unknown fields in strict mode', () => {
      ...
      const validationResult = validatePartial(partialUpdateWithUnknown, sampleSchemas.userSchema, {
        strict: true,
      });

      const validationError = expectFailureError(validationResult);
      expect(validationError.some(e => e.field === 'unknownField' && e.code === 'UNKNOWN')).toBe(true);
    });
  });
  ...
  describe('validateMany - Array Validation', () => {
    it('should reject non-array data', () => {
      ...
      const validationResult = validateMany(nonArrayInput, sampleSchemas.userSchema);
    ...
    it('should reject object instead of array', () => {
      ...
      const validationResult = validateMany(objectInput, sampleSchemas.userSchema);
    ...
    it('should collect errors from multiple items', () => {
      ...
      const validationResult = validateMany(mixedValidityUsers, sampleSchemas.userSchema);
    ...
    it('should abort early on first item error if option is set', () => {
      const usersWithFirstInvalid = [
        { id: 1 },
        validData.user,
      ];

      const validationResult = validateMany(usersWithFirstInvalid, sampleSchemas.userSchema, {
        abortEarly: true,
      });
  ...
  describe('isValid - Boolean Check', () => {
    ...
    it('should respect options', () => {
      ...
      const validStrict = isValid(userWithExtraField, sampleSchemas.userSchema, {
        strict: true,
      });
  ...
  describe('ValidatorOptions - Configuration', () => {
    describe('strict option', () => {
      it('should enforce strict validation by default', () => {
        ...
        const validationResult = validateSchema(userWithExtraField, sampleSchemas.userSchema);
