...
class MockTransaction implements Transaction {
	savepoint(_name: string): Promise<Result<void, TransactionError>> {
		throw new Error("Method not implemented.");
	}
	rollbackTo(_name: string): Promise<Result<void, TransactionError>> {
		throw new Error("Method not implemented.");
	}
	release(_name: string): Promise<Result<void, TransactionError>> {
		throw new Error("Method not implemented.");
	}
...
class MockDatabaseAdapter implements DatabaseAdapter {
	tableExists(_tableName: string): Promise<boolean> {
		throw new Error("Method not implemented.");
	}
	readonly name = "mock";
	readonly config = {};
	...
	async dropTable(tableName: string): Promise<Result<void, QueryError>> {
		if (this._shouldFail && this._failOperation === "dropTable") {
			return { success: false, error: new Error("Drop table failed") as any };
		}
...
class MockMigrationHistory implements MigrationHistory {
	isApplied(_version: string): Promise<Result<boolean, MigrationSystemError>> {
		throw new Error("Method not implemented.");
	}
	calculateChecksum(_migration: Migration): string {
		throw new Error("Method not implemented.");
	}
	verifyChecksum(
		_migration: Migration,
		_record: MigrationHistoryRecord,
	): boolean {
		throw new Error("Method not implemented.");
	}
...
describe("MigrationRunner", () => {
	...
	describe("rollbackLast", () => {
		...
		it("should fail when migration not found", async () => {
			...
			await history.record(
				{
					metadata: { name: "unknown", version: "999", timestamp: Date.now() },
					up: [],
					down: [],
				},
				100,
		...
		it("should fail if target version not found in history", async () => {
			...
			if (!result.success) {
				expect(result.error.message).toContain(
					"not found in migration history",
				);
			}
		});
