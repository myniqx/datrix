...
describe('Core - Dispatcher - Error Path', () => {
  it('should throw if entrance query is invalid', async () => {
    ...
    await expect(dispatcher.dispatchBeforeQuery(invalidEntranceQuery)).rejects.toThrow(
  ...
  it('should throw if plugin returns an invalid query (now strict)', async () => {
    ...
    await expect(dispatcher.dispatchBeforeQuery(validEntranceQuery)).rejects.toThrow(
