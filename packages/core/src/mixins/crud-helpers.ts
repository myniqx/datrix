		...
		if (field?.type === "relation") {
			...
			if (typeof value === "object" && value !== null) {
				...
				if (relInput.disconnect !== undefined) {
					normalizedInput.disconnect = normalizeRelationIds(
						relInput.disconnect,
					);
				}
	...
	for (const [key, value] of Object.entries(data)) {
		...
		if (field?.type === "relation") {
			...
			if (relation.kind === "belongsTo" || relation.kind === "hasOne") {
				...
				if (relData.connect) {
					inlinedId = Array.isArray(relData.connect)
						? relData.connect[0]?.id
						: relData.connect.id;
				} else if (relData.set) {
					inlinedId = Array.isArray(relData.set)
						? relData.set[0]?.id
				...
				if (inlinedId !== undefined) {
					...
					const hasOtherOps =
						relData.create || relData.update || relData.delete;
					if (hasOtherOps) {
						relations[key] = value;
					}
				} else {
