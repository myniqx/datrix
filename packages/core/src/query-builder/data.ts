...
import type { NormalizedNestedData, NormalizedRelationOperations, NormalizedRelationUpdate } from "forja-types/core/query-builder";
...
export function processData<T extends ForjaEntry>(
  ...
  const normalizedRelations: Record<string, NormalizedRelationOperations<T>> = {};
  ...
  for (const [key, value] of Object.entries(rawRelations)) {
    ...
    // Case 3: RelationInput object - normalize each operation to number arrays
      ...
      if (relInput.disconnect !== undefined) {
        normalized = { ...normalized, disconnect: extractIds(relInput.disconnect) };
      }
      ...
      if (relInput.create !== undefined) {
        ...
        if (!targetSchema) {
          throwInvalidValue("data", `relation ${key}`, field.model, "valid model");
        }
    ...
    if (field.kind === "belongsTo" || field.kind === "hasOne") {
      ...
      if (inlinedId !== undefined) {
        ...
        const hasOtherOps = normalized.create || normalized.update || normalized.delete;
        if (hasOtherOps) {
  ...
  return {
    ...
    relations: Object.keys(normalizedRelations).length > 0 ? normalizedRelations : undefined,
  };
}
