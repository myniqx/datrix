...
describe("PostgresAdapter - Connection Management", () => {
	...
	it("should apply default application name (datrix)", async () => {
...
describe("PostgresAdapter - Raw Query Execution", () => {
	...
	it("should execute raw SELECT query", async () => {
		await adapter.executeRawQuery(`INSERT INTO test_raw (data) VALUES ($1)`, [
			"test data",
		]);
	...
	it("should fail on invalid raw SQL", async () => {
		const result = await adapter.executeRawQuery("INVALID SQL SYNTAX", []);
	...
	it("should provide metadata for raw SELECT", async () => {
		...
		const result = await adapter.executeRawQuery("SELECT * FROM test_raw", []);
