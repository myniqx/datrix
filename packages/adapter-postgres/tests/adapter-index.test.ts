...
describe("PostgresAdapter - Index Operations", () => {
	...
	beforeEach(async () => {
		...
		await adapter.executeRawQuery(
			"DROP INDEX IF EXISTS idx_test_index_table_email",
			[],
		);
		await adapter.executeRawQuery('DROP INDEX IF EXISTS "idx-special"', []);
		await adapter.executeRawQuery(
			"DROP INDEX IF EXISTS idx_partial_active",
			[],
		);
	});
	...
	it("should fail to drop index when not connected", async () => {
		...
		const result = await disconnectedAdapter.dropIndex(
			"test_table",
			"idx_name",
		);
	...
	it("should drop index successfully", async () => {
		...
		// Drop it
		const result = await adapter.dropIndex(
			"test_index_table",
			"idx_test_email",
		);
