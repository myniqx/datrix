...
describe("PostgresAdapter - Schema Operations", () => {
	...
	beforeEach(async () => {
		...
		await adapter.executeRawQuery(
			"DROP TABLE IF EXISTS test_schema_parent",
			[],
		);
	});
	...
	it("should create table with required fields (NOT NULL)", async () => {
		...
		expect(result.success).toBe(false);
		if (!result.success) expect(result.error?.message).toContain("null value");
	});
	...
	it("should escape table name when dropping", async () => {
		...
		await adapter.executeRawQuery('CREATE TABLE "select" (id INT)', []);
	...
	it("should fail to alter table when not connected", async () => {
		...
		const result = await disconnectedAdapter.alterTable(
			"test_table",
			operations,
		);
	...
	it("should escape table name in alter operations", async () => {
		// Create table with keyword name
		await adapter.executeRawQuery('CREATE TABLE "where" (id INT)', []);
	...
	it("should include table name in dropTable error message", async () => {
		...
		expect(result.success).toBe(false);
		if (!result.success) expect(result.error?.message).toContain("some_table");
	...
	it("should create table with UNIQUE constraint on single column", async () => {
		...
		await adapter.executeRawQuery(
			"INSERT INTO test_schema_table (id, email) VALUES (1, 'u@e.com')",
			[],
		);
		const result = await adapter.executeRawQuery(
			"INSERT INTO test_schema_table (id, email) VALUES (2, 'u@e.com')",
			[],
		);
	...
	it("should handle multi-column UNIQUE constraints if defined in indexes", async () => {
		const schema: SchemaDefinition = {
			...
			indexes: [{ fields: ["provider", "provider_id"], unique: true }],
		};

		await adapter.createTable(schema);

		await adapter.executeRawQuery(
			"INSERT INTO test_schema_table (provider, provider_id) VALUES ('p1', 'id1')",
			[],
		);
		const result = await adapter.executeRawQuery(
			"INSERT INTO test_schema_table (provider, provider_id) VALUES ('p1', 'id1')",
	...
	it("should support FOREIGN KEY constraints", async () => {
		...
		await adapter.executeRawQuery(
			"ALTER TABLE test_schema_parent ADD PRIMARY KEY (id)",
			[],
		);
		...
		const result = await adapter.executeRawQuery(
			"INSERT INTO test_schema_table (id, parent_id) VALUES (1, 999)",
			[],
		);
		expect(result.success).toBe(false);
		if (!result.success)
	...
	it("should fail to drop table with active dependencies (RESTRICT behavior)", async () => {
		...
		if (!result.success)
			expect(result.error?.message).toMatch(
				/cannot drop table .* because other objects depend on it/i,
			);
	});
	...
	it("should modify column constraints (e.g. adding NOT NULL to existing column)", async () => {
		...
		await adapter.createTable(schema);
		await adapter.executeRawQuery(
			"INSERT INTO test_schema_table (val) VALUES (NULL)",
			[],
		);
