...
import type { Transaction } from '@adapters/base/types';
...
describe('PostgresAdapter - Transaction Lifecycle', () => {
  ...
  it('should fail to rollback already committed transaction', async () => {
    ...
    expect(rollbackResult.error?.message).toContain('Transaction already committed');
  });
  ...
  it('should fail to commit already rolled back transaction', async () => {
    ...
    expect(commitResult.error?.message).toContain('Transaction already rolled back');
  });
  ...
  it('should fail to rollback already rolled back transaction', async () => {
    ...
    expect(secondRollback.error?.message).toContain('Transaction already rolled back');
  });
  ...
  it('should abort transaction and disallow further queries after an error (Postgres behavior)', async () => {
    ...
    expect(subsequentQuery.error?.message).toMatch(/current transaction is aborted, commands ignored until end of transaction block/i);
  ...
  it('should support savepoints for partial rollbacks within a transaction', async () => {
    ...
    const selectResult = await adapter.executeRawQuery<{ value: string }>('SELECT value FROM test_txn', []);
    expect(selectResult.data?.rows).toHaveLength(1);
