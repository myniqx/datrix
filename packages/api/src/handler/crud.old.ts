...
import { createQueryBuilder } from 'forja-core/query-builder/builder';
import { validatePartial, validateSchema } from 'forja-core/validator/schema-validator';
...
export async function findMany<TUser = unknown>(
  ...
  try {
    ...
    if (!queryResult.success) {
      ...
      const errorCode = isValidationError ? 'INVALID_QUERY' : queryResult.error.code;
      return createErrorResponse(status, queryResult.error.message, errorCode);
    }
...
export async function findOne<TUser = unknown>(
  ...
  try {
    ...
    if (!queryResult.success) {
      ...
      const status = isValidationError ? 400 : 500;
      const errorCode = isValidationError ? 'INVALID_QUERY' : 'QUERY_BUILD_ERROR';
      return createErrorResponse(status, queryResult.error.message, errorCode);
    }
