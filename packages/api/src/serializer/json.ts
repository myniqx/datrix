...
>(
	...
	| { success: true; data: SerializedData<T> }
	| { success: false; error: SerializerError } {
	// Validate input
	if (!isRecord(data)) {
		return {
			success: false,
			error: new SerializerError("Data must be an object", {
				code: "INVALID_DATA",
			}),
		};
	}
...
>(
	...
	| { success: true; data: SerializedCollection<T> }
	| { success: false; error: SerializerError } {
	// Validate input
	if (!Array.isArray(data)) {
		return {
			success: false,
			error: new SerializerError("Data must be an array", {
				code: "INVALID_DATA",
			}),
		};
	}
...
>(
	...
	| { success: true; data: SerializedData<T> | SerializedCollection<T> }
	| { success: false; error: SerializerError } {
	if (Array.isArray(data)) {
