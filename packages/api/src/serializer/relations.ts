...
export function serializeRelations(
	...
	try {
		...
		for (const [relationName, populateOptions] of Object.entries(
			options.populate,
		)) {
		...
		return {
			success: false,
			error: new SerializerError(
				error instanceof Error
					? error.message
					: "Relation serialization failed",
				{
...
function serializeRelation(
	...
	| { success: false; error: SerializerError } {
	// Check depth
	if (depth > maxDepth) {
		return {
			success: false,
			error: new SerializerError(
				`Maximum relation depth (${maxDepth}) exceeded`,
				{
					code: "INVALID_RELATION",
				},
			),
	...
	if (
		...
		(fieldDef.kind === "hasMany" || fieldDef.kind === "manyToMany")
	) {
		return serializeRelationArray(
			data,
			fieldDef,
			populateOptions,
			visited,
	...
	return serializeRelationSingle(
		data,
		fieldDef,
		populateOptions,
		visited,
 ...
 */
function serializeRelationWildcard(
	data: unknown,
 ...
 */
function serializeRelationArray(
	data: unknown,
	_fieldDef: FieldDefinition,
	populateOptions: PopulateOptions,
	visited: Set<string>,
 ...
 */
function serializeRelationSingle(
	data: unknown,
	_fieldDef: FieldDefinition,
	populateOptions: PopulateOptions,
	visited: Set<string>,
	...
	return serializeRelationRecord(
		data,
		populateOptions,
		visited,
		depth,
 ...
 */
function serializeRelationRecord(
	data: unknown,
	populateOptions: PopulateOptions,
	visited: Set<string>,
	depth: number,
	...
	if (populateOptions.populate) {
		for (const [nestedRelation, nestedOptions] of Object.entries(
			...
			// Using placeholder field definition - foreignKey not needed for serialization
			const nestedResult = serializeRelation(
				nestedData,
				{ type: "relation", model: "", kind: "hasOne", foreignKey: "" },
				nestedOptions,
				visited,
				depth + 1,
				maxDepth,
