...
export const whereError = {
  ...
  invalidFieldName(
    ...
    throw new ParserError(`Invalid field name in WHERE clause: ${fieldName}${reasonDetail}`, {
  ...
  invalidOperatorValue(
    ...
    const valuePreview = receivedValue !== undefined
...
export const populateError = {
  ...
  invalidFieldName(
    ...
    throw new ParserError(`Invalid field name in populate: ${fieldName}${reasonDetail}`, {
      code: "INVALID_FIELD_NAME",
      parser: "populate",
      location: buildErrorLocation(["populate", ...path]),
      received: fieldName,
...
export const fieldsError = {
  invalidFieldNames(
    ...
    const reasonDetail = context?.validationReasons && context.validationReasons.length > 0
      ? ` (Reasons: ${context.validationReasons.join(", ")})`
      : '';

    throw new ParserError(`Invalid field names: ${invalidFields.join(", ")}${reasonDetail}`, {
      code: "INVALID_FIELD_NAME",
      parser: "fields",
...
export const sortError = {
  ...
  invalidFieldName(field: string, path: string[], context?: Partial<SortErrorContext>) {
    const reasonDetail = context?.fieldValidationReason
      ? ` (Reason: ${context.fieldValidationReason})`
      : '';
