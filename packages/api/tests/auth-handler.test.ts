...
import { createTestConfigWithAuth, testJwtSecret } from './data/config-auth';
...
describe('Auth Handler Tests', () => {
  let forja: Forja;
  const tmpDir = path.join(process.cwd(), 'packages', 'api', 'tests', '.tmp-auth-handler');
  ...
  async function handleRequest(request: Request): Promise<Response> {
    ...
    const response = await (apiPlugin as { handleRequest: (req: Request, forja: Forja) => Promise<Response> }).handleRequest(request, forja);
  ...
  describe('POST /api/auth/register', () => {
    it('should register a new user successfully', async () => {
      ...
      expect((data.data?.user as Record<string, unknown>)['password']).toBeUndefined();
    });
  ...
  describe('POST /api/auth/logout', () => {
    it('should logout successfully with valid session', async () => {
      ...
      const data = (await logoutResponse.json()) as ApiResponse<{ success: boolean }>;
  ...
  describe('GET /api/auth/me', () => {
    ...
    it('should return current user when authenticated with token', async () => {
      ...
      const data = (await response.json()) as ApiResponse<{ email: string; role: string }>;
      expect(data.data).toBeDefined();
