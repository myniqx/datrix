...
describe('JsonAdapter - Happy Path', () => {
  ...
  describe('Invariants: Determinism', () => {
    it('should generate sequential IDs', async () => {
      ...
      const insert1 = expectSuccessData(await adapter.executeQuery({
        type: 'insert',
        table: 'users',
        data: { name: 'First' }
      }));

      const insert2 = expectSuccessData(await adapter.executeQuery({
        type: 'insert',
        table: 'users',
        data: { name: 'Second' }
      }));

      const insert3 = expectSuccessData(await adapter.executeQuery({
        type: 'insert',
        table: 'users',
        data: { name: 'Third' }
      }));
    ...
    it('should return consistent results for same query', async () => {
      ...
      const result1 = expectSuccessData(await adapter.executeQuery({
        type: 'select',
        table: 'users',
        orderBy: [{ field: 'name', direction: 'asc' }]
      }));

      const result2 = expectSuccessData(await adapter.executeQuery({
        type: 'select',
        table: 'users',
        orderBy: [{ field: 'name', direction: 'asc' }]
      }));
    ...
    it('should not be affected by object key order', async () => {
      await adapter.createTable({ name: 'users', fields: { name: { type: 'string', required: true }, age: { type: 'number', required: false } } });

      const data1 = { name: 'Test', age: 25 };
      const data2 = { age: 25, name: 'Test' };

      await adapter.executeQuery({ type: 'insert', table: 'users', data: data1 });
  ...
  describe('Invariants: Data Integrity', () => {
    it('should maintain data after disconnect and reconnect', async () => {
      await adapter.createTable({ name: 'users', fields: { name: { type: 'string', required: true } } });
      await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Persistent' } });
    ...
    it('should not create record on failed insert', async () => {
      ...
      const beforeCount = expectSuccessData(await adapter.executeQuery({
        type: 'select',
        table: 'users'
      }));
      ...
      const afterCount = expectSuccessData(await adapter.executeQuery({
        type: 'select',
        table: 'users'
      }));
