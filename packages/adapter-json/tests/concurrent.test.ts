...
describe('JsonAdapter - Concurrent Access & Race Conditions', () => {
  ...
  describe('Concurrent Writes', () => {
    ...
    it('should handle concurrent updates to different records', async () => {
      await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'User1' } });
      await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'User2' } });
      await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'User3' } });

      const concurrentUpdates = [
        adapter.executeQuery({ type: 'update', table: 'users', where: { id: 1 }, data: { name: 'Updated1' } }),
        adapter.executeQuery({ type: 'update', table: 'users', where: { id: 2 }, data: { name: 'Updated2' } }),
        adapter.executeQuery({ type: 'update', table: 'users', where: { id: 3 }, data: { name: 'Updated3' } })
      ];
    ...
    it('should handle concurrent updates to same record gracefully', async () => {
      await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Original' } });
      ...
      const result = expectSuccessData(
        await adapter.executeQuery({ type: 'select', table: 'users', where: { id: 1 } })
      );
    ...
    it('should handle mixed concurrent operations', async () => {
      await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Existing' } });

      const mixedOperations = [
        adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'New1' } }),
        adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'New2' } }),
        adapter.executeQuery({ type: 'update', table: 'users', where: { id: 1 }, data: { name: 'Modified' } }),
        adapter.executeQuery({ type: 'select', table: 'users' }),
        adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'New3' } })
      ];

      const results = await Promise.all(mixedOperations);

      const allCompleted = results.every(r => r.success === true || r.success === false);
  ...
  describe('Read-Write Race Conditions', () => {
    it('should prevent corruption during simultaneous read-write', async () => {
      ...
      for (let i = 0; i < 20; i++) {
        if (i % 2 === 0) {
          operations.push(
            adapter.executeQuery({ type: 'insert', table: 'users', data: { name: `User${i}` } })
          );
        } else {
          operations.push(
            adapter.executeQuery({ type: 'select', table: 'users' })
      ...
      if (noErrors) {
        ...
        const fileContent = await fs.readFile(path.join(root, 'users.json'), 'utf-8');
        expect(() => JSON.parse(fileContent)).not.toThrow();
      }
    });

    it('should maintain data integrity during concurrent delete-read', async () => {
      await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'ToDelete' } });
      await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'ToKeep' } });

      const operations = [
        adapter.executeQuery({ type: 'delete', table: 'users', where: { id: 1 } }),
        adapter.executeQuery({ type: 'select', table: 'users' }),
        adapter.executeQuery({ type: 'select', table: 'users', where: { id: 2 } })
      ];
  ...
  describe('Determinism & Idempotency', () => {
    ...
    it('should handle concurrent identical queries deterministically', async () => {
      await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Test' } });
  ...
  describe('Lock Behavior', () => {
    ...
    it('should prevent deadlock scenarios', async () => {
      await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Initial' } });

      const operations = Array.from({ length: 10 }, (_, i) => [
        adapter.executeQuery({ type: 'update', table: 'users', where: { id: 1 }, data: { name: `A${i}` } }),
        adapter.executeQuery({ type: 'select', table: 'users', where: { id: 1 } }),
        adapter.executeQuery({ type: 'update', table: 'users', where: { id: 1 }, data: { name: `B${i}` } })
      ]).flat();
