...
describe('JsonAdapter - Advanced Features', () => {
  ...
  beforeEach(async () => {
    ...
    // Setup Tables
    await adapter.createTable({ name: 'users', fields: { name: { type: 'string', required: true }, age: { type: 'number', required: false }, role: { type: 'string', required: true } } });
    await adapter.createTable({ name: 'posts', fields: { title: { type: 'string', required: true }, authorId: { type: 'number', required: true }, views: { type: 'number', required: false } } });
    await adapter.createTable({ name: 'comments', fields: { content: { type: 'string', required: true }, postId: { type: 'number', required: true }, userId: { type: 'number', required: true } } });
  });
  ...
  it('should support SELECT projection', async () => {
    await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Alice', age: 30, role: 'admin' } });
    await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Bob', age: 25, role: 'user' } });
  ...
  it('should support DISTINCT', async () => {
    await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Alice', role: 'admin' } });
    await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Charlie', role: 'admin' } }); // Duplicate role
    await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Bob', role: 'user' } });
  ...
  it('should support COMPLEX WHERE (AND/OR)', async () => {
    await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Alice', age: 30, role: 'admin' } });
    await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Bob', age: 25, role: 'user' } });
    await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Eve', age: 35, role: 'admin' } });
  ...
  it('should support ORDER BY (Complex)', async () => {
    await adapter.executeQuery({ type: 'insert', table: 'posts', data: { title: 'C', views: 10 } });
    await adapter.executeQuery({ type: 'insert', table: 'posts', data: { title: 'A', views: 20 } });
    await adapter.executeQuery({ type: 'insert', table: 'posts', data: { title: 'B', views: 20 } });
  ...
  it('should support NESTED POPULATE', async () => {
    ...
    await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Hacker', role: 'user' } }); // id 1
    await adapter.executeQuery({ type: 'insert', table: 'posts', data: { title: 'Hack', authorId: 1 } }); // id 1
    await adapter.executeQuery({ type: 'insert', table: 'comments', data: { content: 'Nice', postId: 1, userId: 1 } }); // id 1
    ...
    const query: QueryObject = {
      ...
      meta: {
        relations: {
          ...
          author: { // This is for 'post.author' - simplistic flat meta assumption or recursive context?
            // If implementation passes root meta, we need keys to be unique or handled.
            // Assuming JsonPopulator passes QueryObject recursively with SAME meta.
            // JsonPopulator looks up 'author'.
            type: 'belongsTo',
            targetTable: 'users',
            foreignKey: 'authorId',
            kind: 'belongsTo'
  ...
  it('should support HAS_MANY populate', async () => {
    await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: 'Author' } }); // id 1
      ...
      meta: {
        relations: {
          posts: {
            type: 'hasMany',
            targetTable: 'posts',
            foreignKey: 'authorId', // FK on Target
            kind: 'hasMany'
          }
        }
      }
