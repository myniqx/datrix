...
describe('JsonAdapter - Performance & Resource Usage', () => {
  ...
  describe('Memory Management', () => {
    ...
    it('should handle large result sets efficiently', async () => {
      ...
      for (let i = 0; i < 200; i++) {
        await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: `User${i}` } });
      }
  ...
  describe('Query Performance', () => {
    ...
    it('should execute filtered queries efficiently', async () => {
      await adapter.createTable({ name: 'users', fields: { name: { type: 'string', required: true }, role: { type: 'string', required: true } } });
  ...
  describe('Stress Testing', () => {
    ...
    it('should maintain performance under mixed workload', async () => {
      ...
      for (let i = 0; i < 30; i++) {
        await adapter.executeQuery({ type: 'insert', table: 'users', data: { name: `Initial${i}` } });
      }
      ...
      const operations = [];
      for (let i = 0; i < 30; i++) {
        operations.push(adapter.executeQuery({ type: 'select', table: 'users' }));
        operations.push(adapter.executeQuery({ type: 'insert', table: 'users', data: { name: `New${i}` } }));
        operations.push(adapter.executeQuery({ type: 'update', table: 'users', where: { id: i + 1 }, data: { name: `Updated${i}` } }));
      }
