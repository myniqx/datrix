...
describe('JsonAdapter - Locking & Concurrency', () => {
  ...
  beforeEach(async () => {
    ...
    await adapter.createTable({ name: 'counter', fields: { val: { type: 'number', required: true } } });
    // Initial value 0
    await adapter.executeQuery({ type: 'insert', table: 'counter', data: { val: 0 } });
  ...
  it('should serialize concurrent writes to avoid lost updates', { timeout: 10_000 }, async () => {
    ...
    const updates = Array.from({ length: 10 }).map(async (_, i) => {
      ...
      await adapter1.executeQuery({ type: 'insert', table: 'counter', data: { val: i + 1 } });
    });
    ...
    const result = await adapter1.executeQuery({ type: 'select', table: 'counter' });
  ...
  it('should recover from stale locks', async () => {
    const adapter = new JsonAdapter({ root, lockTimeout: 1000, staleTimeout: 500 });
    await adapter.connect();
    ...
    await adapter.executeQuery({ type: 'insert', table: 'counter', data: { val: 999 } });
  ...
  it('should fail if lock cannot be acquired within timeout', async () => {
    const adapter = new JsonAdapter({ root, lockTimeout: 100, staleTimeout: 5000 });
    await adapter.connect();
    ...
    // Should fail
    const result = await adapter.executeQuery({ type: 'insert', table: 'counter', data: { val: 888 } });
