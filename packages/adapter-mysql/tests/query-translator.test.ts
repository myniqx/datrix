...
describe('MySQL Query Translator', () => {
  ...
  describe('Identifier Escaping', () => {
    ...
    it('should reject identifiers exceeding 64 characters', () => {
      ...
      expect(() => translator.escapeIdentifier(longName)).toThrow('exceeds MySQL maximum length');
    });
  ...
  describe('Value Escaping', () => {
    ...
    it('should escape backslashes in strings', () => {
      expect(translator.escapeValue('path\\to\\file')).toBe("'path\\\\to\\\\file'");
    });
  ...
  describe('WHERE Clause Translation', () => {
    ...
    describe('String Operators', () => {
      it('should translate $like operator', () => {
        const result = translator.translateWhere({ name: { $like: '%john%' } }, 0);
      ...
      it('should translate $ilike operator with LOWER()', () => {
        const result = translator.translateWhere({ name: { $ilike: '%JOHN%' } }, 0);
      ...
      it('should translate $contains operator', () => {
        const result = translator.translateWhere({ name: { $contains: 'john' } }, 0);
      ...
      it('should translate $startsWith operator', () => {
        const result = translator.translateWhere({ email: { $startsWith: 'admin' } }, 0);
      ...
      it('should translate $endsWith operator', () => {
        const result = translator.translateWhere({ domain: { $endsWith: '.com' } }, 0);
      ...
      it('should translate $regex operator with REGEXP', () => {
        const result = translator.translateWhere({ code: { $regex: '^[A-Z]{3}$' } }, 0);
      ...
      it('should translate $regex with RegExp object', () => {
        const result = translator.translateWhere({ code: { $regex: /^[A-Z]+$/ } }, 0);
    ...
    describe('NULL Handling', () => {
      ...
      it('should handle $exists operator (true)', () => {
        const result = translator.translateWhere({ avatar: { $exists: true } }, 0);

        expect(result.sql).toContain('`avatar` IS NOT NULL');
      });

      it('should handle $exists operator (false)', () => {
        const result = translator.translateWhere({ avatar: { $exists: false } }, 0);

        expect(result.sql).toContain('`avatar` IS NULL');
      });

      it('should handle $null operator', () => {
        const result = translator.translateWhere({ deletedAt: { $null: true } }, 0);
  ...
  describe('Error Handling', () => {
    ...
    it('should throw error for $in with non-array value', () => {
      expect(() => {
        translator.translateWhere({ role: { $in: 'admin' as unknown as string[] } }, 0);
      }).toThrow('$in operator requires array');
    });

    it('should throw error for $nin with non-array value', () => {
      expect(() => {
        translator.translateWhere({ role: { $nin: 'admin' as unknown as string[] } }, 0);
