...
import { DatrixEntry, DatrixRecord, ParsedQuery } from "datrix-types";
import { queryToParams } from "datrix-api/serializer";

type UseDatrixCollectionReturn<T extends DatrixEntry = DatrixRecord> = {
	data: T[];
	isLoading: boolean;
	error: Error | null;
	refetch: () => void;
...
type UseDatrixSingleReturn<T extends DatrixEntry = DatrixRecord> = {
	data: T | null;
	isLoading: boolean;
	error: Error | null;
	refetch: () => void;
...
export function useDatrix<T extends DatrixEntry = DatrixRecord>(
	modelName: string,
	query?: ParsedQuery<T>,
): UseDatrixCollectionReturn<T>;
export function useDatrix<T extends DatrixEntry = DatrixRecord>(
	modelName: string,
	id: number,
	query?: ParsedQuery<T>,
): UseDatrixSingleReturn<T>;

export function useDatrix<T extends DatrixEntry = DatrixRecord>(
	modelName: string,
	idOrQuery?: number | ParsedQuery<T>,
	maybeQuery?: ParsedQuery<T>,
): UseDatrixCollectionReturn<T> | UseDatrixSingleReturn<T> {
