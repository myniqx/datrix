...
export default function CommentTree({ comments: initialComments, topicId, users, parentId }: CommentTreeProps) {
  ...
  const { data: fetchedComments, isLoading: searching } = useForja<Comment>('comment', commentQuery!);
  ...
  const handleToggleLike = async (comment: Comment) => {
    ...
    const existingLike = comment.likes?.find((l) => l.user?.id === activeUser.id);
          ...
          <div className="bg-slate-50 p-3 rounded-lg group">
            ...
            <div className="flex items-center gap-4 opacity-70 group-hover:opacity-100 transition-opacity">
              <div className="relative group/commentlikes">
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${comment.likes?.some((l) => l.user?.id === users[0]?.id) ? 'fill-red-500' : ''}`} viewBox="0 0 20 20" fill="currentColor">
