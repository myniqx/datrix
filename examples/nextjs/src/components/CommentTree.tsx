...
export default function CommentTree({ comments: initialComments, topicId, users, onUpdate, parentId }: CommentTreeProps) {
  const { create: createComment } = useForja('Comment');
  const { create: createLike, remove: removeLike } = useForja('Like');
  const { data: searchedComments, fetchAll: searchComments, loading: searching } = useForja('Comment');
      ...
      searchComments({
        where: {
          ...
          $or: [
            ...
            { author: { name: { $contains: val } } }
          ]
        },
        populate: {
          author: true,
          replies: { populate: { author: true, likes: { populate: { user: true } } } },
  ...
  const comments = (isRoot && searchTerm) ? searchedComments : initialComments;
  ...
  const handleToggleLike = async (comment: any) => {
    ...
    const existingLike = comment.likes?.find((l: any) => l.user?.id === activeUser.id);
        ...
        <div key={comment.id} className={`pl-4 border-l-2 ${parentId ? 'border-slate-100' : 'border-slate-200'} py-2`}>
          <div className="bg-slate-50 p-3 rounded-lg group">
            <div className="flex justify-between items-center mb-1">
              <span className="text-sm font-bold text-slate-700">{comment.author?.name || 'Anonymous'}</span>
              ...
              <div className="relative group/commentlikes">
                <button
                  ...
                  className={`flex items-center gap-1 text-[11px] font-semibold transition-colors ${
                    comment.likes?.some((l: any) => l.user?.id === users[0]?.id)
                      ? 'text-red-500'
                      : 'text-slate-400 hover:text-red-500'
                  }`}
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 ${comment.likes?.some((l: any) => l.user?.id === users[0]?.id) ? 'fill-red-500' : ''}`} viewBox="0 0 20 20" fill="currentColor">
                  ...
                  <div className="absolute bottom-full left-0 mb-2 hidden group-hover/commentlikes:block z-10 w-48">
                    <div className="bg-slate-900 text-white text-xs rounded-lg p-2 shadow-lg">
                      ...
                      <div className="space-y-1">
                        {comment.likes.slice(0, 5).map((like: any) => (
                          <div key={like.id} className="flex items-center gap-2">
                            <img src={like.user?.avatar || `https://ui-avatars.com/api/?name=${like.user?.name || 'U'}`} className="w-4 h-4 rounded-full" />
                            <span>{like.user?.name || 'Anonymous'}</span>
                          </div>
                        ))}
                        {comment.likes.length > 5 && (
                          <div className="text-slate-400">+{comment.likes.length - 5} more</div>
                        )}
                      </div>
                    </div>
                  </div>
