...
export interface FieldMetadata {
  ...
  readonly relationKind: 'hasOne' | 'hasMany' | 'belongsTo' | 'manyToMany' | undefined;
}
...
/**
 * Generate Zod schema string from schema (for external validation)
 */
export function generateZodSchemaString(
  schema: SchemaDefinition
): string {
  const fields = Object.entries(schema.fields);
  ...
  const fieldStrings = fields.map(([name, field]) => {
    ...
    switch (field.type) {
      ...
      case 'enum': {
        const enumField = field as EnumField;
        const values = enumField.values.map((v) => `'${v}'`).join(', ');
        zodType = `z.enum([${values}])`;
        break;
      }
      case 'array': {
        const arrayField = field as ArrayField;
        const itemZodType = generateZodSchemaString({
          name: 'temp',
          fields: { item: arrayField.items }
        });
