...
const modules = import.meta.glob<DocModule>("/src/docs/**/*.mdx", { eager: true })
...
export function buildDocNav(): DocNavSection[] {
  ...
  for (const section of sectionMap.values()) {
    section.items.sort((a, b) => {
      const aOrder = modules[`/src/docs/${a.slug}.mdx`]?.frontmatter.order ?? 99
      const bOrder = modules[`/src/docs/${b.slug}.mdx`]?.frontmatter.order ?? 99
