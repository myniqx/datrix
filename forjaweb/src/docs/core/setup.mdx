...
Hooks run on every non-raw query for the schema. They fire **after** plugin hooks, and only when the operation goes through the dispatcher (i.e. not on `forja.raw.*` calls).
...
type Post = ForjaEntry & { title: string; slug: string; status: string }
