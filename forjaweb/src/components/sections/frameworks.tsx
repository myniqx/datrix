  ...
  useEffect(() => {
    function tick() {
      ...
      if (trackInitialLeftRef.current === null && trackRef.current) {
        ...
        trackInitialLeftRef.current = trackRef.current.getBoundingClientRect().left
      }
      ...
      ITEMS.forEach((_, i) => {
        ...
        if (!el) return
        const cardCenterX = trackInitialLeft + i * CARD_UNIT + CARD_WIDTH / 2 - offsetRef.current
    ...
    <Container className="py-24">
      ...
      <div className="mb-12 flex flex-col gap-3">
        ...
        <p className="text-base text-foreground/80">
          Forja is framework-agnostic. Use it with any TypeScript backend — no lock-in.
        </p>
      </div>
      ...
      >
        <div ref={trackRef} className="flex items-center will-change-transform" style={{ gap: CARD_GAP }}>
          {ITEMS.map((fw, i) => (
            <div
              key={i}
              ref={(el) => { cardRefs.current[i] = el }}
              className="shrink-0"
              style={{ width: CARD_WIDTH, height: 120, transformOrigin: "center center" }}
            >
              <Card className="h-full">
                <CardContent className="flex h-full items-center gap-3 overflow-hidden px-5 py-4">
                  <div className="shrink-0 text-foreground/70">
                    {fw.icon
                      ? <svg role="img" viewBox="0 0 24 24" className="size-7 fill-current"><path d={fw.icon.path} /></svg>
                      : <WindIcon className="size-7" />
                    }
                  </div>
                  <div className="flex flex-col gap-0.5 min-w-0">
                    <span className="text-sm font-semibold text-foreground truncate">{fw.name}</span>
                    <span className="text-xs text-foreground/80 truncate">{fw.description}</span>
                  </div>
                </CardContent>
              </Card>
            </div>
