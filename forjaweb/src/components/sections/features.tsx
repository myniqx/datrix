...
const ADAPTERS = [
  { name: "PostgreSQL", description: "Full query translation, relations, migrations." },
      ...
      <CardContent className="flex h-full flex-col gap-5 pt-6 pb-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex flex-col gap-1">
            <h3 className="text-base font-semibold text-foreground">Adapter System</h3>
          ...
          {ADAPTERS.map((adapter) => (
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-mono font-semibold text-foreground/90">{adapter.name}</span>
              </div>
              <p className="text-[11px] text-foreground/80 leading-relaxed">{adapter.description}</p>
            </div>
          ))}
        </div>
      </CardContent>
    ...
    <Card>
      <CardContent className="flex h-full flex-col gap-5 pt-6 pb-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex flex-col gap-1">
            <h3 className="text-base font-semibold text-foreground">CLI Tooling</h3>
            <p className="text-sm text-foreground/80 leading-relaxed">
...
const API_FEATURES = [
  { name: "REST Endpoints", description: "Auto-generated CRUD routes for every schema." },
  { name: "Auth (optional)", description: "JWT & session, register/login/logout/me built-in." },
  { name: "Permissions", description: "Role-based access control per route and schema." },
  { name: "Query Parsing", description: "Filter, populate, sort, paginate via URL params." },
] as const
    ...
    <Card>
      <CardContent className="flex h-full flex-col gap-5 pt-6 pb-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex flex-col gap-1">
            <h3 className="text-base font-semibold text-foreground">API Plugin</h3>
            <p className="text-sm text-foreground/80 leading-relaxed">
          ...
          {API_FEATURES.map((feature) => (
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold text-foreground/90">{feature.name}</span>
              </div>
              <p className="text-[11px] text-foreground/80 leading-relaxed">{feature.description}</p>
            </div>
          ))}
        </div>
      </CardContent>
