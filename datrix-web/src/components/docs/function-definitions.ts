...
export const FUNCTION_DEFINITIONS: Record<string, FunctionDefinition> = {
	...
	findOne: {
		signature: `findOne<T extends DatrixEntry>(\n  model: string,\n  where: WhereClause<T>,\n  options?: RawCrudOptions<T>,\n): Promise<T | null>`,
		description:
	...
	findById: {
		signature: `findById<T extends DatrixEntry>(\n  model: string,\n  id: number,\n  options?: RawCrudOptions<T>,\n): Promise<T | null>`,
		description:
	...
	findMany: {
		signature: `findMany<T extends DatrixEntry>(\n  model: string,\n  options?: RawFindManyOptions<T>,\n): Promise<T[]>`,
		description:
	...
	count: {
		signature: `count<T extends DatrixEntry>(\n  model: string,\n  where?: WhereClause<T>,\n): Promise<number>`,
		description:
	...
	createMany: {
		signature: `createMany<T extends DatrixEntry, TInput extends FallbackInput>(\n  model: string,\n  data: TInput[],\n  options?: RawCrudOptions<T>,\n): Promise<T[]>`,
		description:
	...
	update: {
		signature: `update<T extends DatrixEntry, TInput extends FallbackInput>(\n  model: string,\n  id: number,\n  data: TInput,\n  options?: RawCrudOptions<T>,\n): Promise<T>`,
		description:
	...
	updateMany: {
		signature: `updateMany<T extends DatrixEntry, TInput extends FallbackInput>(\n  model: string,\n  where: WhereClause<T>,\n  data: TInput,\n  options?: RawCrudOptions<T>,\n): Promise<T[]>`,
		description:
	...
	delete: {
		signature: `delete<T extends DatrixEntry>(\n  model: string,\n  id: number,\n  options?: RawCrudOptions<T>,\n): Promise<T>`,
		description:
	...
	deleteMany: {
		signature: `deleteMany<T extends DatrixEntry>(\n  model: string,\n  where: WhereClause<T>,\n  options?: RawCrudOptions<T>,\n): Promise<T[]>`,
	...
	getConfig: {
		signature: `getConfig(): DatrixConfig`,
		description:
	...
	beginMigrate: {
		...
		description:
			"Used internally by the Datrix CLI. Compares current schemas against the live database and returns a MigrationSession.",
	...
	defineConfig: {
		signature: `defineConfig(\n  factory: () => DatrixConfig,\n): () => Promise<Datrix>`,
		description:
			"Define Datrix configuration. Returns a function that when called, returns an initialized Datrix instance.",
  ...
  standalone?: boolean,
	...
	ApiPlugin: {
		...
		description:
			"Turns any Datrix instance into a fully-featured REST API. Auto-generates CRUD routes for every schema, handles authentication, and optionally manages file uploads.",
	},
	...
	queryToParams: {
		signature: `queryToParams<T extends DatrixEntry = DatrixRecord>(\n  query: ParsedQuery<T> | undefined,\n): string`,
		description:
	...
	serializeQuery: {
		signature: `serializeQuery<T extends DatrixEntry = DatrixEntry>(\n  query: ParsedQuery<T>,\n): RawQueryParams`,
		description:
