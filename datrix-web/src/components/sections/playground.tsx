...
const { schemas, groups } = playgroundData as unknown as {
	schemas: Schema[];
	groups: Group[];
};
		...
		return (
			<>
				...
				{value.map((item, i) => (
					<span key={i}>
						{innerPad}
						<CodeArg value={item} indent={indent + 2} />
		...
		return (
			<>
				...
				{entries.map(([k, v], i) => (
					<span key={k}>
						{innerPad}
						<span style={{ color: semanticKeyColor(k) }}>{k}</span>
						<span style={S.punct}>{": "}</span>
...
function CodeBlock({ scenario }: { scenario: Scenario }) {
	...
	const fn = (
		<>
			<span style={S.kw}>datrix</span>
			<span style={S.punct}>.</span>
			<span style={S.fn}>{action}</span>
		</>
	);
			...
			{showJson ? (
				<pre className="text-xs font-mono leading-relaxed">
					<code>
						...
						<JsonToken
							value={{ name: schema.name, fields: schema.fields }}
							indent={0}
						/>
				...
				fields.map(([fieldName, def]) => {
					...
					return (
						...
						>
							...
							<span
								style={{
									color: FIELD_TYPE_COLORS[def.type] ?? CODE_COLORS.plain,
								}}
							>
...
const GROUP_COLORS: Record<string, string> = {
	...
	delete:
		"data-[state=active]:text-destructive data-[state=active]:border-destructive",
};
...
export function Playground() {
	...
	const currentScenario =
		currentGroup.scenarios.find((s) => s.id === activeScenario) ??
