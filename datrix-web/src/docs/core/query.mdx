...
> **Note:** `datrix.findMany` and `datrix.crud.findMany` are identical.
> `datrix.raw.findMany` behaves the same but skips plugin hooks (no `onBeforeQuery` / `onAfterQuery` dispatch).
...
> `limit`/`offset` on a **hasMany**/**manyToMany** populate apply per parent record — `populate: { comments: { limit: 5 } }` returns up to 5 comments for *each* post, not 5 total across the whole result set.
...
```typescript
// Deduplicate — requires an explicit select
await datrix.findMany("user", {
	select: ["role"],
	distinct: true,
});

// Group by a scalar field, filter groups with having
await datrix.findMany("user", {
	select: ["role"],
	groupBy: ["role"],
	having: { role: { $ne: "guest" } },
});
```

`groupBy` only accepts scalar fields — relation fields cannot be grouped on. `having` can only reference fields already present in `groupBy`. For grouped counts, use `datrix.countMany` instead of `findMany` — see [countMany](/docs/core/interfaces#countmany).
...
See [CLI: generate types](/docs/cli#datrix-generate-types) for output path and options.
