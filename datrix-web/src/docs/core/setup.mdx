...
export const userSchema = defineSchema({
	...
	permission: {
		...
		read: true,
		// ctx.user.id is the authentication record's id — compare against
		// ctx.user.user.id (the populated user record) to check "own profile"
		update: (ctx: PermissionContext) => ctx.user?.user?.id === ctx.id,
...
```typescript
import { defineSchema } from "@datrix/core"
import type { DatrixEntry, QueryContext } from "@datrix/core"
import type {
  QueryInsertObject,
  QueryUpdateObject,
  QueryDeleteObject,
...
type Post = DatrixEntry & { title: string; status: "draft" | "published" }
