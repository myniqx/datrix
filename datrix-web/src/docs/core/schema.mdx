---
title: Schema
description: Defining and registering schemas in Datrix.
order: 2
---
...
Schemas are plain TypeScript objects that describe your data models. Datrix uses them to
...
Relations describe how schemas connect to each other. Datrix automatically manages
...
<FlowChain>
	<FlowStep>onBeforeQuery (plugins)</FlowStep>
	<FlowStep>before* (schema)</FlowStep>
	<FlowStep>[query runs]</FlowStep>
	<FlowStep>onAfterQuery (plugins)</FlowStep>
	<FlowStep>after* (schema)</FlowStep>
</FlowChain>
...
<StepList>
	<Step title="Register internal metadata schema">
		_datrix table for internal bookkeeping
	</Step>
	<Step title="Register user schemas">
		All schemas from your config
	</Step>
	<Step title="Register plugin schemas">
		Plugins can contribute their own schemas (e.g. soft-delete adds deletedAt)
	</Step>
	<Step title="Apply schema extensions">
		Plugins can add/remove/modify fields on existing schemas
	</Step>
	<Step title="Register migration schema">
		_datrix_migration table for migration history
	</Step>
	<Step title="Finalize registry">
		Processes all relations (adds FK columns, creates junction tables), sorts
		schemas by dependency order
	</Step>
	<Step title="Initialize plugins">
		Plugins receive the final schema state
	</Step>
	<Step title="Dispatch schema load event">
		Plugins can react to the finalized schema registry
	</Step>
</StepList>
