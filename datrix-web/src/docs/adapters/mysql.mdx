...
Two strategies are selected automatically based on query depth and complexity:
...
- **Batched IN Queries** — used for deep nesting (depth > 1), high cardinality, or populate options like `limit`, `offset`, `where`, `orderBy`. Collects parent IDs and issues targeted `WHERE id IN (...)` queries, stitching results in Node.js memory.

`limit`/`offset` on a `hasMany`/`manyToMany` populate apply **per parent record**, not globally across the batch.
