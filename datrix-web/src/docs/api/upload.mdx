...
<FeatureGrid>
	<FeatureCard icon={UploadIcon} title="POST /api/upload" compact>
		Upload a file (multipart/form-data).
	</FeatureCard>
	<FeatureCard icon={Trash2Icon} title="DELETE /api/upload/:id" compact>
		Delete a file and its variants.
	</FeatureCard>
	<FeatureCard icon={SearchIcon} title="GET /api/upload" compact>
		List media records (pagination, filtering, populate).
	</FeatureCard>
	<FeatureCard icon={FileSearchIcon} title="GET /api/upload/:id" compact>
...
Media records are immutable over HTTP once created — there is no `PATCH` endpoint. To change a file, delete it and upload a new one; this keeps storage and the media record from drifting apart.

**New in 0.2.0:** `POST`/`DELETE` now go through the same permission check as CRUD (`checkSchemaPermission`, including the D2 default) against the media schema's `create`/`delete` permission — previously they were open to anyone. With no explicit `permission`, an authenticated user is required. Configure it via `permission` on `UploadOptions` (see [Configuration reference](#configuration-reference)).
...
```json
{
  "data": {
    ...
    "variants": {
      "thumbnail": { "key": "1704067200000-abc123-thumbnail.webp", "url": "...", "width": 150, "height": 150, "size": 4100, "mimeType": "image/webp" },
      "small":     { "key": "1704067200000-abc123-small.webp", "url": "...", "width": 320, "height": 213, "size": 12800, "mimeType": "image/webp" }
    }
  }
}
```
...
For `image/*` types, the declared MIME type is verified against the actual file content (via `sharp`) — a mismatched file is rejected rather than stored under a false type. Non-image types are not content-verified; the declared type is trusted as-is, so avoid serving them inline from a public bucket without `Content-Disposition: attachment` / `X-Content-Type-Options: nosniff`.

---

## Media schema

When upload is configured, `ApiPlugin` automatically registers a `media` schema with these fields. By default the schema's `create`/`update` permission is `false` (blocked over HTTP) — media records are only meant to be written through the upload pipeline, not plain CRUD. Pass `permission` in `UploadOptions` to override.
