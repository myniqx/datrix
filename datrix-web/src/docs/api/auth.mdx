...
new ApiPlugin<Role>({
	auth: {
		...
		passwordReset: {
			tokenExpirySeconds: 3600, // default: 1 hour

			onForgotPassword: async (user: AuthenticatedUser<Role>, token) => {
				// user contains the full authenticated record (password fields excluded)
				// send the token however you like — email, Telegram, SMS, etc.
				await myMailer.send({
					to: user.email,
					subject: "Reset your password",
					body: `https://example.com/reset?token=${token}`,
				});
			},
		},
...
Requires `passwordReset.onForgotPassword` to be configured — otherwise the endpoint returns `403`. Always responds with `{ data: { success: true } }` regardless of whether the email exists (prevents user enumeration).
...
Verifies the token and updates the password. The token expires after `passwordReset.tokenExpirySeconds` seconds (default: 1 hour).

```http
POST /api/auth/reset-password
Content-Type: application/json

{ "token": "a3f9...", "password": "newSecret123" }
```
...
When a schema has no `permission` and `defaultPermission` doesn't cover the action: `read` is allowed for everyone, and `create`/`update`/`delete` require an authenticated user (an anonymous request is rejected, not allowed).

### `ctx.user` shape

`ctx.user` is the authentication record, not the user record — its `id` is the auth row's id, not the id your `authorId`/`userId` foreign keys point to. The populated user record is available at `ctx.user.user`:
...
ctx.user satisfies {
  ...
  email: string
  role:  Role
  user:  DatrixRecord     // the populated user record — compare FKs against ctx.user.user.id
} | undefined
```
...
permission: {
  update: [
    ...
    async (ctx) => {
      ...
      return record?.authorId === ctx.user?.user?.id
    },
...
auth: {
  endpoints: {
    ...
    me:              "/auth/me",               // default
    forgotPassword:  "/auth/forgot-password",  // default
    resetPassword:   "/auth/reset-password",   // default
...
| `forgotPassword`  | `string`  | `"/auth/forgot-password"`    | Forgot password endpoint path.       |
| `resetPassword`   | `string`  | `"/auth/reset-password"`     | Reset password endpoint path.        |
| `disableRegister` | `boolean` | `false`                      | Block all new user registrations.    |

### `auth.passwordReset`

Password reset is disabled by default. Set `onForgotPassword` to enable it.
...
auth: {
  passwordReset: {
    ...
    onForgotPassword: async (user: AuthenticatedUser<Role>, token: string) => {
      // user contains the full authenticated record (password fields excluded)
      // use token to build your reset link, then notify however you prefer
      await myMailer.send({
        to: user.email,
        subject: "Reset your password",
        body: `https://example.com/reset?token=${token}`,
...
| Option               | Type                                              | Default  | Description                                                                             |
| -------------------- | ------------------------------------------------- | -------- | --------------------------------------------------------------------------------------- |
| `tokenExpirySeconds` | `number`                                          | `3600`   | Reset token lifetime in seconds.                                                        |
| `onForgotPassword`   | `(user: AuthenticatedUser, token: string) => Promise<void>` | —  | Called after a token is generated. Use it to send an email, Telegram message, or any notification. Required to activate the endpoint. |
