...
For every schema registered with Datrix, `ApiPlugin` generates the following routes. The `:schema` segment matches the schema's table name (e.g. schema `"product"` → `/api/products`).

<FeatureGrid>
	<FeatureCard icon={SearchIcon} title="GET /api/:schema" compact>
		List records with pagination, filtering, sorting, populate.
	</FeatureCard>
	...
	<FeatureCard
		...
		href="/docs/api/crud#complex-queries-via-query--post-alias"
		compact
	>
...
<FeatureGrid>
	<FeatureCard icon={LayersIcon} title="CRUD" href="/docs/api/crud">
		Filtering, pagination, sorting, populate.
	</FeatureCard>
	<FeatureCard icon={ShieldIcon} title="Auth" href="/docs/api/auth">
		JWT & session authentication, roles, permissions.
	</FeatureCard>
	<FeatureCard icon={UploadIcon} title="Upload" href="/docs/api/upload">
		File uploads, storage providers, image variants.
	</FeatureCard>
</FeatureGrid>
