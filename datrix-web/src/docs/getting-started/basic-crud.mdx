...
`updateMany` requires a `where` clause, just like `deleteMany` — this prevents accidentally updating every row. To intentionally update the whole table, pass an empty condition explicitly: `datrix.updateMany('user', {}, { role: 'user' })`.
