...
description: How to build a Datrix plugin.
order: 1
---
...
<StepList>
	<Step title="init(context)">
		Called once with adapter, schemas, and config
	</Step>
	<Step title="getSchemas()">
		Plugin schemas registered into the registry
	</Step>
	<Step title="extendSchemas(ctx)">
		Field/index extensions applied to existing schemas
	</Step>
	<Step title="Finalize registry">
		Relations resolved, junction tables created
	</Step>
	<Step title="onSchemaLoad(schemas)">
		Called after registry is fully ready
	</Step>
</StepList>
...
<FlowChain>
	<FlowStep>onCreateQueryContext</FlowStep>
	<FlowStep>onBeforeQuery</FlowStep>
	<FlowStep>[query runs]</FlowStep>
	<FlowStep>onAfterQuery</FlowStep>
</FlowChain>
